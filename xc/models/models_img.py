...
class ImgEncoderBase(EncoderBase):
    ...
    def set_pretrained(self):
        return PreTrainedImg(self, self.params)


class PreTrainedImg(EncoderBase):
    def __init__(self, model, params):
        self.model_dim = model.fts
        model = Projection(model.fts, model.fts)
        super(PreTrainedImg, self).__init__(model, params.project_dim)
        self.apply_pooling = True
...
class ModelViT(ImgEncoderBase):
    def __init__(self, model, transf, params):
        ...
        self.features.pooler = nn.Identity()
...
class ModelBeiT(ImgEncoderBase):
    def __init__(self, model, transf, params):
        super(ModelBeiT, self).__init__(model, transf, params.project_dim)

    def encode(self, images):
        images = self.transform(images)
        images = self.features(images, return_dict=True).pooler_output
    ...
    def freeze_params(self):
        super().freeze_params()
        for params in self.features.encoder.layer[-2:].parameters():
            params.requires_grad = True
        for params in self.features.layernorm.parameters():
            params.requires_grad = True
        for params in self.features.pooler.parameters():
            params.requires_grad = True
...
class ModelCLIP(ImgEncoderBase):
    def __init__(self, model, transf, params):
        super(ModelCLIP, self).__init__(model, transf, params.project_dim)

    def encode(self, images):
        if isinstance(self.features, nn.Linear):
            return self.features(images).unsqueeze(1)
        images = self.transform(images)
        images = self.features["model"](images)[1]
        images = self.features["project"](images)
        return images.unsqueeze(1)
    ...
    def freeze_params(self):
        pass
        super().freeze_params()
        for layer in self.features.encoder.layers[-2:]:
