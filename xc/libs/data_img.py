...
def load_img(root, n_file, max_worker_thread=10, random_k=-1):
    try:
...
class IMGBINDataset(IMGDatasetBase):
    def __init__(self, root, n_file, max_thread=10,
                 random_k=-1, img_file="images/img.bin"):
        super().__init__(root, n_file, random_k)
        self.max_thread = max_thread
        self.vect = os.path.join(root, img_file)
    ...
    def build_pre_trained(self, img_model, data_dir, file_name,
        ...
        pre_trained = DataParallel(Model(img_model, params))
