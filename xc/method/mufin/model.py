...
class Mufin(MufinModelBase):
    ...
    def predict_shorty(self, data_dir, tst_img, tst_txt, tst_lbl,
        ...
        tst_shorty = os.path.join(self.params.result_dir,
                                  "module2", tst_shorty)
        shorty = ut.load_file(tst_shorty)

        docs = self.extract_item(tst_dset.X, "docs").mean_pooled.cpu().numpy()
        lbl_embs = self.anns.lbls_emb
        lbl_embs = ut.normalize(lbl_embs)
        shorty = padded_inputs(shorty.tocsr())
        index = shorty['index'].cpu().numpy()
        mask = shorty['mask'].cpu().numpy()
        idx = np.array_split(np.arange(len(tst_dset)),
                             self.params.batch_size)
        scores = []
        for _idx in ut.pbar(idx):
            _docs = docs[_idx][:, None, :]
            _lbls = lbl_embs[index[_idx]]
            _mask = mask[_idx]
            _scrs = np.sum(_docs*_lbls, axis=-1)
            scores.append(_scrs*_mask)
        scores = np.vstack(scores)
        score_mat = sp.lil_matrix(tst_dset.gt.shape)
        idx = np.arange(len(tst_dset)).reshape(-1, 1)
        score_mat[idx, index] = scores
        score_mat = {"emb": score_mat.tocsr()}
    ...
    def extract(self, data_dir, tst_img, tst_txt):
        ...
        return super().extract_modal(tst_dset)
    ...
    def train(self, trn_dset, tst_dset):
        ...
        for epoch in np.arange(ws, ne):
            ...
            if (epoch) % 5 == 0:
                docs, lbls = self.callback(trn_dset)
                if epoch > 150:
                    self.adjust_leaf_sz(trn_dl.b_size//2, epoch-150, ne-150)
                trn_dl.dataset.callback_(lbls, docs, self.params)
                if tst_dset is not None:
                    score_mat = self._predict(tst_dset.X)
                    self.evaluate(score_mat, tst_dset.gt)
            self.save(self.params.model_dir, "model.pkl")
        if tst_dset is not None:
...
class MufinRanker(MufinModelBase):
    ...
    def _predict(self, tst_dset, X=None, L=None):
        ...
        if os.environ['RESTRICTMEM'] == '1':
            ...
            clf_wts = self.net.preset_weights(L.mean_pooled)
            shorty = tst_dset.module2
            # clf_shorty = self.anns.gpu_ova(X.mean_pooled.numpy(), clf_wts)
            # shorty = clf_shorty.maximum(shorty)
        ...
        # score["module4/clf_shorty"] = clf_shorty
        # score["module4/shorty"] = shorty
        return score
    ...
    def callback(self, trn_dset, L=None):
        ...
        # doc_xf = X.mean_pooled.cpu().numpy()
        # trn_dset.callback_(doc_xf, self.params)
        trn_dset.mode = "train"
        return trn_dset
